...
# Input events embedded in the stream: {"input":"key","name":"j"}
...
# $1 = input JSON lines (trace loaded separately from inputs)
drive() {
    printf '%s\n' "$1" > "$TMPDIR/input.jsonl"
    "$TV" --load "$TRACE" --trace "$TMPDIR/input.jsonl" 2>&1
}
...
drive_trace() {
    printf '%s\n' "$2" > "$TMPDIR/input.jsonl"
    set +e
    "$TV" --load "$1" --trace "$TMPDIR/input.jsonl" > "$TMPDIR/drive.out" 2>&1
    DRIVE_RC=$?
    set -e
...
# ═══════════════════════════════════════════════════════════════════════
# Test: separate trace+input files via --load and --trace
# ═══════════════════════════════════════════════════════════════════════
printf '%s\n' \
...
OUT=$("$TV" --load "$TRACE" --trace "$TMPDIR/input_only.jsonl" 2>&1)
run_test "separate_streams: --load trace --trace input works" \
    'assert_contains t "$OUT" "foo.c"' \
    'assert_contains t "$OUT" "bar.c"' \
